...
class TestDockerIntegration:
    ...
    def config_manager(
        ...
        try:
            config_manager = ServerConfigManager(
                base_configs_path=TestDirectories.CONFIG_MANAGER_CONFIGS_DIR,
                base_configs_file=TestDirectories.CONFIG_MANAGER_CONFIGS_FILE,
            )
    ...
    def arserver_controller(
        ...
        try:
            arserver_controller = ServerController(
                config_manager=config_manager,
                root_path=TestDirectories.CONTROLLER_DIR,
                container_name_prefix=constants.SERVER_PREFIX,
                docker_client=constants.DOCKER_CLIENT,
            )
        except Exception as e:
            raise IntegrationTestError(
                f"Failed to initialize ServerController: {e}"
            ) from e

        if not arserver_controller.initialize_directories():
            raise IntegrationTestError(
                "Failed to initialize directories for ServerController"
            )
        ...
        def tracked_create_server(*args, **kwargs) -> bool:
            """Chama `ServerController.create_server` e guarda `server_name` em uma lista para persistencia dos nomes."""
        ...
        for path in os.listdir(TestDirectories.CONTAINER_VOLUMES_DIR):
            shutil.rmtree(f"{TestDirectories.CONTAINER_VOLUMES_DIR}/{path}", True)
    ...
    def test_docker_client_initialization(self, arserver_controller: ServerController):
        # Act
    ...
    def test_server_creation(self, arserver_controller: ServerController):
        # Act
        # user_role: str = "admin"
        ...
        assert isinstance(config, ServerConfigType), (
    ...
    def test_server_creation_stores_container_id(
        ...
        assert server_config.arserver_config.container_id is not None, (
        ...
        assert container_id == server_container.id, (
            "'container_id' must be the same as `Container.id` got from Docker client."
        )

    def test_server_removal(self, arserver_controller: ServerController):
        # Act
        user_role: str = "admin"
    ...
    def test_list_servers(self, arserver_controller: ServerController):
        # Act
        user_role: str = "admin"
        server_1: str = TestUtils.generate_unique_server_name("list")
        server_2: str = TestUtils.generate_unique_server_name("list")
        config_server_1: str = (
            f"{TestDirectories.DS_CONFIGS_DIR}/testServerConfig_1.json"
        )
        config_server_2: str = (
            f"{TestDirectories.DS_CONFIGS_DIR}/testServerConfig_2.json"
        )
        ...
        assert all(isinstance(server, ARServer) for server in servers), (
            "All items should be ARServer instances"
        )

        assert len(servers) == 2, "Expected exactly two servers in the list"

        assert any(server.server_name == f"{server_1}" for server in servers), (
            "Expected server 1 to be in the list"
        )
        assert any(server.server_name == f"{server_2}" for server in servers), (
            "Expected server 2 to be in the list"
        )

    def test_server_start(self, arserver_controller: ServerController):
        # Act
        user_role: str = "admin"
    ...
    def test_server_servernotfound_error(self, arserver_controller: ServerController):
        # Act
        user_role: str = "admin"
    ...
    def test_server_throws_error_on_invalid_config(
        self, arserver_controller: ServerController
    ):
    ...
    def test_server_stop(self, arserver_controller: ServerController):
        # Act
        user_role: str = "admin"
    ...
    def test_server_restart(self, arserver_controller: ServerController):
        # Act
        user_role: str = "admin"
    ...
    def test_server_status(self, arserver_controller: ServerController):
        # Act
        user_role: str = "admin"
